...
- **Устойчивость очереди:** каждый файл запускается в отдельном дочернем процессе; авария нативного инференса для одного файла не должна закрывать GUI целиком.
- **Resume без ручной сверки:** если для файла уже есть оба результата (`.txt` + `.json`), файл автоматически пропускается.
...
В GUI каждый файл также запускается в изоляции процесса для защиты от аварийного завершения нативных библиотек. При необходимости можно задать отдельный лимит времени на файл через `TRANSCRIBATOR_GUI_FILE_TIMEOUT_SECONDS`.
...
| `TRANSCRIBATOR_GUI_FILE_TIMEOUT_SECONDS` | gui/core | >0 — лимит времени на один файл в GUI-очереди (каждый файл уже запускается изолированно). |
...
### Ошибка `did not find executable at 'C:\Python314\python.exe'`

Windows-окружение `.venv` зависит от базового Python, путь к которому записан в
`.venv\pyvenv.cfg`. Если базовый Python удалить, запуск через
`.venv\Scripts\python.exe` и `Запуск Transcribator.bat` перестанет работать, даже
если все пакеты внутри `.venv` сохранились.

Для текущего окружения ожидаемый путь — `C:\Python314\python.exe`. Проверенный
способ восстановления:

1. Установить Python той же minor-версии (`3.14.x`) строго в `C:\Python314`.
2. Проверить окружение: `.\.venv\Scripts\python.exe --version`.
3. Проверить зависимости: `.\.venv\Scripts\python.exe -m pip check`.
4. Запустить `Запуск Transcribator.bat` и выполнить тестовую транскрибацию.

Нельзя просто направить существующее окружение на Python другой minor-версии:
бинарные пакеты внутри `.venv` собраны под конкретную версию Python (например,
`cp314`). Альтернатива — полностью пересоздать `.venv` и переустановить
зависимости, включая NVIDIA CUDA-пакеты.

После перемещения папки проекта скрипты ручной активации `.venv\Scripts\activate*`
могут содержать старый абсолютный путь. BAT-файл запускает Python напрямую и от
этого не зависит.

| Проблема | Что проверить |
|----------|----------------|
| `did not find executable at 'C:\Python314\python.exe'` | Установлен ли Python 3.14.x строго в `C:\Python314`; затем проверить `.venv\Scripts\python.exe --version`. |
