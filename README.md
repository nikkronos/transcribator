...
- **Диаризация (опционально):** пакет `sherpa-onnx` (ставится из `requirements.txt`). Считается на CPU и не требует GPU/torch. При первом использовании один раз скачиваются ONNX-модели (~46 МБ) в `%LOCALAPPDATA%\Transcribator\models`.
...
Откроется окно: добавь файлы (аудио/видео), при необходимости укажи папку для результатов, **модель** и **устройство** (`auto` / `cuda` / `cpu`), при желании включи галочку **«Диаризация (кто говорит)»**, нажми «Запустить транскрибацию». Лог и **прогресс (процент, ETA, очередь)** — в том же окне.

> При включённой диаризации первый запуск один раз скачивает модели (~46 МБ) в `%LOCALAPPDATA%\Transcribator\models`. Если файл уже обработан ранее (есть `.txt` + `.json`), он будет пропущен — чтобы пере-обработать с диаризацией, удали старые результаты или укажи другую папку.
...
# Диаризация: разметить, кто говорит (Спикер 1, Спикер 2…)
python -m transcribator созвон.mp3 --diarize

# Диаризация с известным числом спикеров (точнее)
...
Поля `diarization` (bool) и `num_speakers` (int) есть всегда; поле `speaker` у сегментов добавляется только при включённой диаризации. Без диаризации `diarization: false`, `num_speakers: 0`, а сегменты содержат только `start`, `end`, `text` (обратная совместимость со старым форматом).
