...
- **Проверка:** e2e на 5-мин клипе реального файла с `--speakers 2` — чистое чередование 2 спикеров, совпадает с логикой Q&A интервью (интервьюер/гостья).

### 2026-06-18 (word-level слияние — точность атрибуции)
- **Симптом:** на полном файле грубое слияние «один спикер на сегмент whisper» путало границы (вопрос интервьюера прилипал к ответу гостьи, непрерывная речь рвалась). Авто-определение числа спикеров признано ненадёжным (число плывёт с длиной) → надёжный путь — задавать число.
- **Решение (как в WhisperX):** при диаризации включаются **пословные таймкоды** whisper (`word_timestamps=True`), спикер назначается **на каждое слово**, и сегмент whisper режется в точке смены говорящего. Новый helper `core._segments_by_speaker`; в `diarization.py` выделен `speaker_label(start,end,turns)` (overlap + ближайший fallback), `assign_speakers` упрощён.
- **Проверка:** юнит-тест разбиения (сегмент со сменой спикера режется на 2 корректные реплики; fallback для сегмента без слов) — прошёл. Полный 94-мин файл прогнан реальным пайплайном с `--speakers 2`.
- **Сглаживание crosstalk:** word-level в местах перебивания давал обрывки в 1-2 слова. Добавлен `core._smooth_short_turns`: короткая реплика (<1с) между более длинными репликами другого спикера переназначается соседу, затем consecutive same-speaker мёржатся. На реальном файле 1015 → 173 реплики, чтение стало чистым. Юнит-тест пройден.
- **Нюанс ctranslate2 (важно для отладки):** на Windows+CUDA процесс может падать (exit 127) **на выгрузке** при финализации модели — ПОСЛЕ записи файлов, т.е. безвредно. В GUI это уже обработано: файл считается в изолированном подпроцессе, и при ненулевом коде выхода, но валидных `.txt`+`.json`, результат принимается (`_outputs_look_valid`). Прямые скрипты доставки разбивались на 2 коротких шага (диаризация→turns; транскрибация+слияние с записью до выгрузки).
